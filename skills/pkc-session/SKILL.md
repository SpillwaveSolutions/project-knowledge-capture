---
name: pkc-session
description: Open or close an isolated second-brain write session (worktree + PR). Use before capturing or materializing when multiple agents share one knowledge remote.
---
...
Export `SECOND_BRAIN_ROOT` and `BRAIN_SESSION_ID` from the JSON. All writes go to that bundle via `scripts/pkc_*.py`.
