...
7. Releases bump the version in **eight** places: `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `.grok-plugin/marketplace.json`, `.codex-plugin/plugin.json`, `.cursor-plugin/plugin.json`, root `plugin.json`, `marketplace.json`, `package.json`, and the README table — plus a `CHANGELOG.md` entry. `TestManifestVersions` fails on any drift between the manifests.
